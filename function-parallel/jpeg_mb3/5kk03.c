...
int vld_count = 0;
volatile unsigned int *cmem_out = (unsigned int *) (mb3_cmemout0_BASEADDR+sizeof(int)+2*sizeof(struct from3_to_2));
...
unsigned int FGETC ( JPGFile *fp )
{
    ...
    hw_dma_receive(cmem_out, &(fp->data[count/4]), 1, &dma, BLOCKING);
    //while(hw_dma_status(&dma));
