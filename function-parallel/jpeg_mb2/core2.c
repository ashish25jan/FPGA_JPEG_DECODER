...
int main (void)
{
  ...
  t1 = hw_tifu_systimer_get();
   ...
   // VldHeader header;
    ...
    //SubHeader2 sh2;
    struct from3_to_2 re_from3;
    struct from2_to_1 se_to1;
    while(1)
    {
        //FValue fv[10];
        //SubHeader1 sh1;
        ...
        fifo_set_consumption_token_rate(fcb_R, 1);
		    ...
		    volatile struct from3_to_2 *sp_R = fifo_claim_data(fcb_R);
		    fifo_pull(fcb_R);
 	      re_from3 = *sp_R;
        ...
        {
            FBlock fbout;
            iqzz( &(re_from3.fv[i]), &fbout);
            idct( &fbout, &(se_to1.pbout[i]));
        }
        
        se_to1.sh1=re_from3.sh1;
        se_to1.sh2=re_from3.sh2;
        
        fifo_set_production_token_rate(fcb_S, 1);
		    while(!fifo_check_space(fcb_S));
		    volatile struct from2_to_1 *sp_S = fifo_claim_space(fcb_S);
		    *sp_S = se_to1;
		    fifo_push(fcb_S);
